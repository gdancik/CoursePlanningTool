...
def create_schedule(term, year, days, url = 'https://www.easternct.edu/academics/academic-calendar/index.html') :
    ...
    # First pass: process calendar to get start/end dates (without week numbers)
    df_calendar = process_academic_calendar(df_calendar, target, year)
    start, end = get_start_and_end_dates(df_calendar)
    
    # Second pass: add week numbers if needed
    if by_week:
        ...
        # Update the Day column with week numbers for calendar entries
...
# def get_final_exam_date(soup, target_semester):
#     '''Returns the final exam date for the target semester'''
#     curr_semester = soup.find_all(class_='main')
#     if curr_semester[0].text.strip() != target_semester :
#     # check if header == Term + Year
...
# Check for matching days i.e. MWF

# Need to pass in class time o.e. 9:00 - 9:50 am

# Only need to check columns where class days fall on i.e. if MWF class don't need to check TR
...
def combine_date_dfs(df1, df2, start, end) :
    ...
    # Standardize df1 columns to match expected format
    if 'Week #' in df1.columns and 'Week of' in df1.columns:
        # Schedule DataFrame with by_week=True format
        df1_standardized = df1.copy()
        df1_standardized = df1_standardized.rename(columns={'Week #': 'Day', 'Week of': 'Date'})
    else:
        # Schedule DataFrame with normal format
        df1_standardized = df1.copy()
    
    # Ensure df1 has all required columns that df2 has
    if 'Date2' not in df1_standardized.columns:
        df1_standardized['Date2'] = ''

    # combine columns
    combined = pd.concat([df1_standardized, df2])
...
def generate_schedule(start_date, end_date, days):
    ...
    # Change the headers based on if it's by week or by day
    if not by_week:
...
if __name__ == "__main__":
    ...
    schedule = create_schedule(term, year, days)

    # Print first 10 rows of the schedule
    print(schedule)
