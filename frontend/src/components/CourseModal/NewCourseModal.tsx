...
interface CourseModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (data: FormState) => Promise<CreatedCourseResult | void>;
  modalTitle: string;
  modalMessage: string;
...
const NewCourseModal: React.FC<CourseModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const createdCourse = await onCreate(formData);

      if (!createdCourse?.course_id) {
        throw new Error("Course was created, but no course_id was returned.");
      }

      onClose();
      navigate(`/courses/${createdCourse.course_id}/overview`);
    } catch (err: unknown) {
